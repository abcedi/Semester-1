...
/****************************************
  ...
  INSERT YOUR NEW MID= FORMULA after the original near LINE 99, 
  also CHANGE the printf statement near line 59 to reflect your new formula
*****************************************/
...
// *****************************************************************************************************
	...
	do  // search between low & high for target
	{
		...
		the intermediate result is cast to an 8-bit integer to cause overflow 
		using only 2^7 iterations instead of the 2^15 or 2^31 iterations using an integer's default width.

		(int8_t)(intermediate_result) will cast to 8-bit for demo and testing formula for overflow
		                 _MID_BUG__ 
  		mid = (int8_t) ( low + high ) / 2 ; 
		...
		Hint: instead of finding the average of low+high as in the bug version, find the middle point between low and high.
		0------------------low--------high-----MAX
						   |-------------|
		...
		*/
		...
		// *****************************************************************************************************
		// calc mid-point with binary search bug (but forcing overflow for any intermediate result > INT8_MAX)
		mid = (int8_t)(low + high) / 2; 
		// your new calc for mid which avoids intermediate value overflow
		//	mid = (int8_t)( /* REMOVE COMMENT // AND INSERT YOUR NEW MID FORMULA HERE */ ) ;	
		// *****************************************************************************************************
